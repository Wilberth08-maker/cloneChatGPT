...
const SearchMenu = ({setSearchTerm, onNewChat, onChatSelect, onDeleteChat, chats, searchTerm, searchRef, currentChatId, onMenuSearchClose, setIsMenuSearchOpen, setIsMobileOpen}) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50">
            <div ref={searchRef} className="bg-[#ffffff] rounded-2xl shadow-lg p-2 h-[65%] w-1/2 max-w-2xl dark:bg-gray-800 dark:text-gray-200 border border-gray-700 flex flex-col">
                ...
                <button
                    ...
                    className="w-full flex items-center justify-between w-full p-2.5 mt-2 rounded-xl text-gray-900 hover:bg-[#ebebeb] transition-colors duration-200 gap-2 dark:text-gray-200 dark:hover:bg-gray-800"
                >
                    ...
                    {filteredChats.length === 0 ? (
                        ...
                        filteredChats.map(chat => (
                            <div key={chat.id} className="flex items-center justify-between mb-2">
                                <button
                                    className={`flex gap-2 itemList items-center block flex-grow text-left p-2 rounded-xl text-gray-900 transition-colors duration-200 ${currentChatId === chat.id ? 'bg-gray-700' : 'hover:bg-[#ebebeb] dark:text-gray-200 dark:hover:bg-gray-800'
                                        }`}
                                    onClick={() => {
